...
module.exports = function (configuration, isInstallDeps) {
  ...
  BoiAuxInstall(isInstallDeps, Dependencies).then(() => {
    ...
    if (Conf_Compile.style.sprites) {
      App.use(`/${Path.basename(Conf_Compile.image.output)}`, Express.static(Path.join(process.cwd(), Conf_Compile.basic.output, Conf_Compile.image.output)));
    }

    // serve static thirdparty libraries
    if (Conf_Compile.basic.libs) {
